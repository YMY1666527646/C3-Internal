...
namespace rend {
	...
	ImGuiWindow& BeginScene() {
		...
		ImGui::SetWindowSize(ImVec2(500.f, 500.f), ImGuiCond_Always);
	...
	VOID EndScene(ImGuiWindow& window) 
	{
		...
		if (Settings.TeleportToEnemies) //must be done in render cuz it will crash on loop
		{
			...
			{
				if (GetAsyncKeyState(Settings.EnemyTeleportKey))
				{
					if (InVehicle)
					{
		...
		if (showMenu)
		{
			...
			if (tab == 0)
			{
				...
				ImGui::Checkbox(xorstr("Box##checkbox"), &Settings.ESP.Boxes);
				...
				ImGui::Checkbox(xorstr("Chams##checkbox"), &Settings.ESP.TestChams);

				ImGui::Combo(xorstr("Box Style"), &boxmodepos, boxmodes, sizeof(boxmodes) / sizeof(*boxmodes));
				ImGui::Combo(xorstr("Snapline Location"), &Settings.SnaplineStartPoint, linemodes, sizeof(linemodes) / sizeof(*linemodes));
				...
				ImGui::Checkbox(xorstr("Guns##checkbox"), &Settings.ESP.Weapons);
				ImGui::Checkbox(xorstr("Chest##checkbox"), &Settings.ESP.Containers);
			...
			else if (tab == 1)
			{
				...
				ImGui::Checkbox(xorstr("Enable (Smooth must be at 1000)##checkbox"), &Settings.Aimbot);
				ImGui::Checkbox(xorstr("FOV##checkbox"), &Settings.ESP.AimbotFOV);
				ImGui::Checkbox(xorstr("Line To Target##checkbox"), &Settings.TargetLine);
				ImGui::Checkbox(xorstr("Vis Check##checkbox"), &Settings.VisibleCheck);			
				ImGui::Checkbox(xorstr("Sniper TP##checkbox"), &Settings.BulletTP);
			...
			//	ImGui::Checkbox(xorstr("vehicle teleporter##checkbox"), &Settings.TeleportToEnemies);
				...
				ImGui::Checkbox(xorstr("Color Changer##checkbox"), &Settings.ColorAdjuster);
				ImGui::Checkbox(xorstr("Debug##checkbox"), &Settings.Info);
			...
			//	ImGui::Checkbox(xorstr("object debug##checkbox"), &Settings.ESP.debug);
				...
				ImGui::Text("I know this is neutron, were back!");

				ImGui::Checkbox(xorstr("Show FPS##checkbox"), &Settings.FPS);
	...
	HRESULT __stdcall hk_present(IDXGISwapChain* pSwapChain, UINT sync_interval, UINT flags)
	{
		...
		if (!device) {
			...
			m_pFont = ImGui::GetIO().Fonts->AddFontFromFileTTF(xorstr("C:\\Windows\\Fonts\\Tahoma.ttf"), 16.0f);
		...
		if (Settings.WaterMark)
		{
			...
			DrawOutlinedText(m_pFont, TextFormat(xorstr("")), ImVec2(120, 5), 16.f, WaterMarkColor, true);
		}
		...
		uint64_t BaseAddress = (uint64_t)GetModuleHandleA(NULL);
			...
			do {
				...
				auto localCameraManager = ReadPointer(localPlayerController, 0x2C0);
				if (!localCameraManager) break;

				Util::GetPlayerViewPoint((uintptr_t)localCameraManager, &Util::CameraLocation, &Util::CameraRotation);


				auto FpsColorcac = ImGui::GetColorU32({ color_red, color_green, color_blue, 255 });
				DrawOutlinedText(m_pFont, xorstr("kakoooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooo"), ImVec2(100, 500), 16.0f, FpsColorcac, false);
				...
				auto SquadID = ReadPointer(localPlayerState, 0x10ac);
				if (!SquadID) break;


				auto FpsColorcaccc = ImGui::GetColorU32({ color_red, color_green, color_blue, 255 });
				DrawOutlinedText(m_pFont, xorstr("Neutron"), ImVec2(100, 125), 32.0f, FpsColorcaccc, false);
				...
				auto weaponName = L"";
				auto isProjectileWeapon = wcsstr(weaponName, xorstr(L"Rifle_Sniper"));
				auto isRocketLauncher = wcsstr(weaponName, xorstr(L"RocketLauncher"));
				auto isGrenadeLauncher = wcsstr(weaponName, xorstr(L"GrenadeLauncher"));
				auto isCrossbow = wcsstr(weaponName, xorstr(L"Crossbow"));
				auto isBoomBow = wcsstr(weaponName, xorstr(L"ExplosiveBow"));
				...
				for (auto li = 0UL; li < ReadDWORD(world, offsets::World::Levels + sizeof(PVOID)); ++li) {
					...
					for (auto ai = 0UL; ai < ReadDWORD(level, offsets::Level::AActors + sizeof(PVOID)); ++ai) {
						...
						if (!actors) break;
						auto pawn = ReadPointer(actors, ai * sizeof(PVOID));
						...
						else if (Settings.ESP.Memes)
						{
							if (wcsstr(name.c_str(), xorstr(L"Rock")))
							{
								...
								if (RockRoot) {
									...
									RockPos = Util::WorldToScreen(width, height, RockPos);
									float dist = Util::SpoofCall(sqrtf, dx * dx + dy * dy + dz * dz) / 100.0f;
							...
							else if (wcsstr(name.c_str(), xorstr(L"Tree")) || wcsstr(name.c_str(), xorstr(L"Pine")))
							{
								...
								if (TreeRoot) {
									...
									TreePos = Util::WorldToScreen(width, height, TreePos);
									float dist = Util::SpoofCall(sqrtf, dx * dx + dy * dy + dz * dz) / 100.0f;
							...
							else if (wcsstr(name.c_str(), xorstr(L"Bush")))
							{
								...
								if (BushRoot) {
									...
									BushPos = Util::WorldToScreen(width, height, BushPos);
									float dist = Util::SpoofCall(sqrtf, dx * dx + dy * dy + dz * dz) / 100.0f;
				...
				for (auto pawn : playerPawns) {
					...
					FVector neck = { 0, 0, 0 };
					Util::GetBoneLocation(pawn, 65, neck);

					FVector chest = { 0, 0, 0 };
					Util::GetBoneLocation(pawn, 36, chest);
					...
					Util::GetBoneLocation(pawn, 9, leftShoulder);

					FVector rightShoulder = { 0, 0, 0 };
					Util::GetBoneLocation(pawn, 62, rightShoulder);

					FVector leftElbow = { 0, 0, 0 };
					Util::GetBoneLocation(pawn, 10, leftElbow);

					FVector rightElbow = { 0, 0, 0 };
					Util::GetBoneLocation(pawn, 38, rightElbow);

					FVector leftHand = { 0, 0, 0 };
					Util::GetBoneLocation(pawn, 11, leftHand);

					FVector rightHand = { 0, 0, 0 };
					Util::GetBoneLocation(pawn, 39, rightHand);

					// Legs
					FVector leftLeg = { 0, 0, 0 };
					Util::GetBoneLocation(pawn, 67, leftLeg);

					FVector rightLeg = { 0, 0, 0 };
					Util::GetBoneLocation(pawn, 74, rightLeg);

					FVector leftThigh = { 0, 0, 0 };
					Util::GetBoneLocation(pawn, 73, leftThigh);

					FVector rightThigh = { 0, 0, 0 };
					Util::GetBoneLocation(pawn, 80, rightThigh);

					FVector leftFoot = { 0, 0, 0 };
					Util::GetBoneLocation(pawn, 68, leftFoot);

					FVector rightFoot = { 0, 0, 0 };
					Util::GetBoneLocation(pawn, 75, rightFoot);

					FVector leftFeet = { 0, 0, 0 };
					Util::GetBoneLocation(pawn, 71, leftFeet);

					FVector rightFeet = { 0, 0, 0 };
					Util::GetBoneLocation(pawn, 78, rightFeet);

					FVector leftFeetFinger = { 0, 0, 0 };
					Util::GetBoneLocation(pawn, 72, leftFeetFinger);

					FVector rightFeetFinger = { 0, 0, 0 };
					Util::GetBoneLocation(pawn, 79, rightFeetFinger);
				...
				if (Settings.Aimbot)
				{
					if (closestPawn && GetAsyncKeyState(Settings.AimKey) && Util::SpoofCall(GetForegroundWindow) == hWnd)
					{					
